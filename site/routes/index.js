...
router.get('/cat/:id', checkToken, function(req, res) {
    pool.query('Select * from categories', (error, cat_res) => {
        if (error) {
            console.log(error.message);
        } else {
            pool.query(`select 
                ...
                on products.id = product_compositions.product_id
                inner join ingredients 
                on product_compositions.ingredient_id = ingredients.id
                where products.category_id = $1`, [req.params.id], (error, prod_res) => {
                if (error) {
                    console.log(error.message);
                } else {
                    if(prod_res.rows.length > 0){
                        var j = 0;
                        products = [{
                            id: prod_res.rows[0].id,
                            name: prod_res.rows[0].name,
...
router.get('/checkout', checkToken, (req, res) => {
